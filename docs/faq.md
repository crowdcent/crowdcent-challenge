...
The meta-model typically represents an aggregation (e.g., an average or ensemble) of all valid user submissions for past inference periods within a specific challenge. It can serve as a benchmark or potentially as an additional feature for your own models. You can download it via the client or CLI, or run it through the [Simulator](simulator.md) as a long/short portfolio without writing any code. Real-time predictions are available to active **Challenger+** participants (100+ CC Points and a submission in the last 30 days); all other users receive 90-day delayed data. Performance scores are open to everyone. See [CC Points](points-system.md) for tier details.

### Can I trade the meta-model?

You can simulate it today: the [Simulator](simulator.md) backtests the meta-model as a long/short Hyperliquid portfolio with configurable optimizer, risk, and cadence controls, and higher [CC Point](points-system.md) tiers unlock more of them. [Live Trading](live-trading.md) through CrowdCent requires Challenger tier (100+ CC Points) and a submission in the last 30 days. Nothing here is investment advice; see the [disclaimer](disclaimer.md).
