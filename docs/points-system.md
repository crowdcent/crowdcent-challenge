...
At the end of the day, all your valid slots are evaluated against the target metrics. We calculate the **composite percentile** of all your evaluated slots (based on [raw metrics](hyperliquid-ranking.md#raw-metrics) only), then apply a **cosine curve** to this percentile. A separate [unique composite percentile](hyperliquid-ranking.md#unique-composite-percentile) exists for uniqueness metrics but does not currently affect points.
