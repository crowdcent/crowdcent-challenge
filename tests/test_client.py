...
def client():
    ...
    return ChallengeClient(challenge_slug=TEST_SLUG, api_key=TEST_API_KEY, base_url=BASE_URL)
...
def test_client_init_with_api_key():
    ...
    client = ChallengeClient(challenge_slug=TEST_SLUG, api_key=TEST_API_KEY, base_url=BASE_URL)
...
def test_client_init_with_env_var(monkeypatch):
    ...
    assert f"Api-Key env_api_key" in client.session.headers["Authorization"]
...
def test_client_init_with_dotenv(mock_load_dotenv, monkeypatch):
    ...
    assert f"Api-Key dotenv_api_key" in client.session.headers["Authorization"]
...
def test_client_init_key_priority(mock_load_dotenv, monkeypatch):
    ...
    monkeypatch.setenv("CROWDCENT_API_KEY", "env_api_key")
    client_direct = ChallengeClient(challenge_slug=TEST_SLUG, api_key="direct_key", base_url=BASE_URL)
...
def test_get_challenge_success(client, requests_mock):
    ...
    assert requests_mock.last_request.headers["Authorization"] == f"Api-Key {TEST_API_KEY}"

def test_get_challenge_not_found(client, requests_mock):
    """Test handling of 404 error when getting challenge details."""
    mock_url = f"{BASE_URL}/challenges/{TEST_SLUG}/"
    requests_mock.get(mock_url, status_code=404, json={"error": {"message": "Not Found"}})
...
def test_list_all_challenges_success(requests_mock, monkeypatch):
    ...
    mock_data = [{"name": "Challenge 1", "slug": "c1"}, {"name": "Challenge 2", "slug": "c2"}]
    requests_mock.get(mock_url, json=mock_data)

    challenges = ChallengeClient.list_all_challenges(base_url=BASE_URL)
    assert challenges == mock_data
    assert requests_mock.last_request.headers["Authorization"] == "Api-Key class_method_key"
...
def test_list_all_challenges_auth_error(requests_mock):
    ...
    mock_url = f"{BASE_URL}/challenges/"
    requests_mock.get(mock_url, status_code=401, json={"error": {"message": "Invalid key"}})
...
def test_request_error_handling(client, requests_mock, status_code, exception_type, error_message):
    """Test the unified error handling for different HTTP status codes."""
    mock_url = f"{BASE_URL}/challenges/{TEST_SLUG}/"
    requests_mock.get(mock_url, status_code=status_code, json={"error": {"code": "TEST_CODE", "message": "Detailed error"}})
...
def test_download_training_dataset_success(client, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(client, "_request", lambda *a, **k: _DummyStreamResponse(b"bytes"))
...
def test_download_inference_data_success_current(client, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(client, "_request", lambda *a, **k: _DummyStreamResponse(b"xyz"))
...
def test_download_inference_data_invalid_date_raises(client, tmp_path):
    ...
    with pytest.raises(ClientError):
        client.download_inference_data(release_date="13-2025-01", dest_path=str(tmp_path / "x.parquet"))