...
def patch_get_client(mock_client):
    ...
    with patch("crowdcent_challenge.cli.get_client", return_value=mock_client) as patched:
...
def test_list_challenges_success(runner, patch_get_client):
    ...
    mock_data = [{"name": "Challenge A", "slug": "a"}, {"name": "Challenge B", "slug": "b"}]
...
def test_get_challenge_success(runner, mock_client):
    ...
    mock_data = {"name": "Mock Challenge", "slug": TEST_SLUG, "description": "Details..."}
    mock_client.get_challenge.return_value = mock_data

    result = runner.invoke(cli, ["get-challenge", TEST_SLUG])
    assert result.exit_code == 0
    assert json.loads(result.output) == mock_data
...
def test_get_challenge_not_found(runner, mock_client):
    ...
    result = runner.invoke(cli, ["get-challenge", "nonexistent-slug"])
    assert result.exit_code != 0
...
def test_list_training_data_success(runner, mock_client):
    mock_data = [{"version": "1.0", "is_latest": True}, {"version": "0.9", "is_latest": False}]
    mock_client.list_training_datasets.return_value = mock_data
    result = runner.invoke(cli, ["list-training-data", TEST_SLUG])
    assert result.exit_code == 0
    assert json.loads(result.output) == mock_data
...
def test_download_training_data_success(runner, mock_client, tmp_path):
    ...
    version = "1.0"
    result = runner.invoke(cli, ["download-training-data", TEST_SLUG, version, "-o", str(output_file)])
    assert result.exit_code == 0
    assert f"Training data downloaded successfully to {output_file}" in result.output
    mock_client.download_training_dataset.assert_called_once_with(version, str(output_file))
...
def test_download_training_data_default_output(runner, mock_client, tmp_path, monkeypatch):
    ...
    result = runner.invoke(cli, ["download-training-data", TEST_SLUG, version])
    assert result.exit_code == 0
    # Assert based on the relative path generated by the CLI
    mock_client.download_training_dataset.assert_called_once_with(version, expected_relative_output)

def test_download_training_data_api_error(runner, mock_client, tmp_path):
    output_file = tmp_path / "training_data.parquet"
    version = "1.0"
    mock_client.download_training_dataset.side_effect = NotFoundError("Dataset version not found")
    result = runner.invoke(cli, ["download-training-data", TEST_SLUG, version, "-o", str(output_file)])
    assert result.exit_code != 0
...
def test_submit_success(runner, mock_client, mock_predictions_file):
    ...
    result = runner.invoke(cli, ["submit", TEST_SLUG, mock_predictions_file])
    assert result.exit_code == 0
    assert "Submission successful!" in result.output
    assert json.loads(result.output.split("\n", 1)[1]) == mock_response # Check JSON part
    mock_client.submit_predictions.assert_called_once_with(mock_predictions_file)

def test_submit_file_not_found(runner):
    # Click's path type validation happens before our command, so expect Click's error
    result = runner.invoke(cli, ["submit", TEST_SLUG, "nonexistent.parquet"])
    assert result.exit_code != 0
...
def test_submit_api_error(runner, mock_client, mock_predictions_file):
    ...
    mock_client.submit_predictions.side_effect = ClientError("Invalid submission format")
    result = runner.invoke(cli, ["submit", TEST_SLUG, mock_predictions_file])
    assert result.exit_code != 0
